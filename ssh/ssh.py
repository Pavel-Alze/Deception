...
def handle_cmd(cmd, chan, ip):
    ...
    if cmd.startswith("ls"):
        ...
        if (cmd[4:] == "README.txt"):
            ...
            response = "cat:" + cmd[4:] + ": No such file or directory"
    else:
...
#         return paramiko.AUTH_SUCCESSFUL
    ...
    def check_auth_password(self, username, password):
        ...
        if(password == USER_PASSWORD and USER_VALID_NAME == username):
#            self.userN = username
            return paramiko.AUTH_SUCCESSFUL
        else:
...
def handle_connection(client, addr):
    ...
    try:
        ...
        try:
            chan.send("Welcome to SSHHostServer\r\n\r\n")
            run = True
            while run:
                chan.send(SSH_FIRST_WORLD)
                command = ""
                while not command.endswith("\r"):
                    ...
                    if (
                        ...
                        if( transport == BACK_KEY ):
                            deleteworld = " " * len(SSH_FIRST_WORLD + command)
                            chan.send("\r" + deleteworld)
                            command = command[:-1]
                            chan.send("\r" + SSH_FIRST_WORLD + command)
